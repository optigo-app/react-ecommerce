...
const FilterAccordion = ({ title, items, checkedItems, handleCheckboxChange }) => {
  return (
    <Accordion className='smrmo_cart-Accordion' style={{ boxShadow: 'none', border: 'none' }}>
        ...
        {items?.map(item => (
          <div className='smrmo_filter-AccrodianDetails' key={item.id}>
            ...
            <Checkbox
              ...
              className='smrmo_filterCheckbox'
            />
          </div>
        ))}
      </AccordionDetails>
...
const SelectedModal = ({ open, onClose, selectedItems, onRemove, onUpdateCart, onCancelCart }) => {
  ...
  return (
    ...
    >
      <div className="smrmo_paper">
        <div className='smrmo_Modal_Title'>
          <Typography className='smrmo_Modal_TitleTypo' variant="h5" id="modal-title" gutterBottom>
            Update Cart
          </Typography>
          <IconButton className="smrmo_closeIcon" onClick={onClose}>
            <CloseIcon />
          </IconButton>
        </div>
        <Divider />
        <div className='smrmo_cartmodal_body'>
          <div>
            <div className="smrmo_ShortCutCombo-section">
              <div>
          ...
          <div className='smrmo_cartModal-Divider'>
            <Divider />
          </div>
          <Grid container spacing={2}>
            <Grid item xs={6} md={3}>
              <div className='smrmo_Modal-FilterList'>
                {generateFilterAccordions(filterArr, checkedItems, handleCheckboxChange)}
              </div>
            </Grid>
            <Grid item xs={6} md={9}>
              <div className='smrmo_Modal-cardList'>
                <Grid container spacing={2}>
                  {filteredItems?.map(product => (
                    <Grid item key={product.id} xs={12} sm={4} md={4}>
                      <Card className='smrmo_cartListCard' sx={{ maxWidth: 250, position: 'relative' }}>
                        <CardActionArea>
                          <CardMedia
                            component="img"
                          ...
                          <CardContent>
                            ...
                            <div className='designNocartList'>
                              <p className='smrmo_DesignNoTExt'>{product?.designno}</p>
                            </div>
          ...
          <CardActions className='smrmo_projectUpdateCartBtn-group'>
            <div className="smrmo_projectUpdateCartBtn-group">
              <button className="smrmo_cartUpdate-btn" onClick={() => onUpdateCart(filteredItems)}>
                Update
              </button>
              <button className="smrmo_cartCancel-btn" onClick={onCancelCart}>
                Cancel
              </button>
            </div>
          </CardActions>
