...
const FilterAccordion = ({ title, items, checkedItems, handleCheckboxChange }) => {
  return (
    <Accordion className='Hoq3_cart-Accordion' style={{ boxShadow: 'none', border: 'none' }}>
      <AccordionSummary expandIcon={<ExpandMoreIcon />} style={{ padding: '0' }}>
        <Typography className='Hoq3_FilterTitle'>{title}</Typography>
      </AccordionSummary>
      <AccordionDetails className='Hoq3_filterAccordianDetail' style={{ padding: '0' }}>
        {items?.map(item => (
          <div className='Hoq3_filter-AccrodianDetails' key={item.id}>
            <Typography className='Hoq3_filterAccordianDeatilTitle'>{item.name}</Typography>
            <Checkbox
              checked={checkedItems[title]?.[item.id] || false}
              onChange={() => handleCheckboxChange(item.id, title)}
              className='Hoq3_filterCheckbox'
            />
          </div>
        ))}
      </AccordionDetails>
...
const MyModal = ({ open, onClose, selectedItems, onRemove, onUpdateCart, onCancelCart }) => {
  ...
  return (
    ...
    >
      <div className="Hoq3_paper">
        <div className='Hoq3_Modal_Title'>
          <Typography className='Hoq3_Modal_TitleTypo' variant="h5" id="modal-title" gutterBottom>
            Update Cart
          </Typography>
          <IconButton className="Hoq3_closeIcon" onClick={onClose}>
            <CloseIcon />
          </IconButton>
        </div>
        <Divider />
        <div className='Hoq3_cartmodal_body'>
          <div>
            <div className="Hoq3_ShortCutCombo-section">
              <div>
          ...
          <div className='Hoq3_cartModal-Divider'>
            <Divider />
          </div>
          <Grid container spacing={2}>
            <Grid item xs={6} md={3}>
              <div className='Hoq3_Modal-FilterList'>
                {generateFilterAccordions(filterArr, checkedItems, handleCheckboxChange)}
              </div>
            </Grid>
            <Grid item xs={6} md={9}>
              <div className='Hoq3_Modal-cardList'>
                <Grid container spacing={2}>
                  {filteredItems?.map(product => (
                    <Grid item key={product.id} xs={12} sm={4} md={4}>
                      <Card className='Hoq3_cartListCard' sx={{ maxWidth: 250, position: 'relative' }}>
                        <CardActionArea>
                          <CardMedia
                            component="img"
                            ...
                            className='Hoq3_cartListImage'
                          />
                          <CardContent>
                            <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap' }}>
                              <div>
                                <Typography variant="body2" className='Hoq3_card-ContentData'>
                                  NWT: {product?.netwt}
                                </Typography>
                                <Typography variant="body2" className='Hoq3_card-ContentData'>
                                  DWT: {product?.dwt}
                            ...
                            <div className='designNocartList'>
                              <p className='Hoq3_DesignNoTExt'>{product?.designno}</p>
                            </div>
          ...
          <CardActions className='Hoq3_projectUpdateCartBtn-group'>
            <div className="Hoq3_projectUpdateCartBtn-group">
              <button className="Hoq3_cartUpdate-btn" onClick={() => onUpdateCart(filteredItems)}>
                Update
              </button>
              <button className="Hoq3_cartCancel-btn" onClick={onCancelCart}>
                Cancel
              </button>
            </div>
          </CardActions>
