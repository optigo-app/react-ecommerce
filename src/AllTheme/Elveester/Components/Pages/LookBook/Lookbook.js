...
const Lookbook = () => {
  ...
  const handleAddToCart = (ele, type) => {
    ...
    CartAndWishListAPI(type, prodObj, cookie)
      .then((res) => {
        ...
        broadcast('UPDATE_CART_COUNT', cartC , ele?.autocode , "cart", true );
      })
      .catch((err) => console.log("err", err));
  };
  ...
  const handleRemoveCart = async (ele) => {
    try {
      ...
      let cartC = res?.Data?.rd[0]?.Cartlistcount;
      setCartCountVal(cartC);
   broadcast('UPDATE_CART_COUNT', cartC , ele?.autocode , "cart", false );
                                      ...
                                      >
                                        ...
                                        <img
                                          ...
                                          onError={(e) => {
                                            ...
                                            e.target.src = imageNotFound; // set fallback image
                                          }}
                                          draggable={false}
                                          onContextMenu={(e) => e.preventDefault()}
                                        />
