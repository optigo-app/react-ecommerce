...
const FilterAccordion = ({ title, items, checkedItems, handleCheckboxChange }) => {
  return (
    <Accordion className='mala3_cart-Accordion' style={{ boxShadow: 'none', border: 'none' }}>
      <AccordionSummary expandIcon={<ExpandMoreIcon />} style={{ padding: '0' }}>
        <Typography className='mala3_FilterTitle'>{title}</Typography>
      </AccordionSummary>
      <AccordionDetails className='mala3_filterAccordianDetail' style={{ padding: '0' }}>
        {items?.map(item => (
          <div className='mala3_filter-AccrodianDetails' key={item.id}>
            <Typography className='mala3_filterAccordianDeatilTitle'>{item.name}</Typography>
            <Checkbox
              checked={checkedItems[title]?.[item.id] || false}
              onChange={() => handleCheckboxChange(item.id, title)}
              className='mala3_filterCheckbox'
            />
          </div>
        ))}
      </AccordionDetails>
...
const MyModal = ({ open, onClose, selectedItems, onRemove, onUpdateCart, onCancelCart }) => {
  ...
  return (
    ...
    >
      <div className="mala3_paper">
        <div className='mala3_Modal_Title'>
          <Typography className='mala3_Modal_TitleTypo' variant="h5" id="modal-title" gutterBottom>
            Update Cart
          </Typography>
          <IconButton className="mala3_closeIcon" onClick={onClose}>
            <CloseIcon />
          </IconButton>
        </div>
        <Divider />
        <div className='mala3_cartmodal_body'>
          <div>
            <div className="mala3_ShortCutCombo-section">
              <div>
          ...
          <div className='mala3_cartModal-Divider'>
            <Divider />
          </div>
          <Grid container spacing={2}>
            <Grid item xs={6} md={3}>
              <div className='mala3_Modal-FilterList'>
                {generateFilterAccordions(filterArr, checkedItems, handleCheckboxChange)}
              </div>
            </Grid>
            <Grid item xs={6} md={9}>
              <div className='mala3_Modal-cardList'>
                <Grid container spacing={2}>
                  {filteredItems?.map(product => (
                    <Grid item key={product.id} xs={12} sm={4} md={4}>
                      <Card className='mala3_cartListCard' sx={{ maxWidth: 250, position: 'relative' }}>
                        <CardActionArea>
                          <CardMedia
                            component="img"
                            ...
                            className='mala3_cartListImage'
                          />
                          <CardContent>
                            <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap' }}>
                              <div>
                                <Typography variant="body2" className='mala3_card-ContentData'>
                                  NWT: {product?.netwt}
                                </Typography>
                                <Typography variant="body2" className='mala3_card-ContentData'>
                                  DWT: {product?.dwt}
                            ...
                            <div className='designNocartList'>
                              <p className='mala3_DesignNoTExt'>{product?.designno}</p>
                            </div>
          ...
          <CardActions className='mala3_projectUpdateCartBtn-group'>
            <div className="mala3_projectUpdateCartBtn-group">
              <button className="mala3_cartUpdate-btn" onClick={() => onUpdateCart(filteredItems)}>
                Update
              </button>
              <button className="mala3_cartCancel-btn" onClick={onCancelCart}>
                Cancel
              </button>
            </div>
          </CardActions>
