...
const FilterAccordion = ({ title, items, checkedItems, handleCheckboxChange }) => {
  return (
    <Accordion className='dt_cart-Accordion' style={{ boxShadow: 'none', border: 'none' }}>
      <AccordionSummary expandIcon={<ExpandMoreIcon />} style={{ padding: '0' }}>
        <Typography className='dt_FilterTitle'>{title}</Typography>
      </AccordionSummary>
      <AccordionDetails className='dt_filterAccordianDetail' style={{ padding: '0' }}>
        {items?.map(item => (
          <div className='dt_filter-AccrodianDetails' key={item.id}>
            <Typography className='dt_filterAccordianDeatilTitle'>{item.name}</Typography>
            <Checkbox
              checked={checkedItems[title]?.[item.id] || false}
              onChange={() => handleCheckboxChange(item.id, title)}
              className='dt_filterCheckbox'
            />
          </div>
        ))}
      </AccordionDetails>
...
const MyModal = ({ open, onClose, selectedItems, onRemove, onUpdateCart, onCancelCart }) => {
  ...
  return (
    ...
    >
      <div className="dt_paper">
        <div className='dt_Modal_Title'>
          <Typography className='dt_Modal_TitleTypo' variant="h5" id="modal-title" gutterBottom>
            Update Cart
          </Typography>
          <IconButton className="dt_closeIcon" onClick={onClose}>
            <CloseIcon />
          </IconButton>
        </div>
        <Divider />
        <div className='dt_cartmodal_body'>
          <div>
            <div className="dt_ShortCutCombo-section">
              <div>
          ...
          <div className='dt_cartModal-Divider'>
            <Divider />
          </div>
          <Grid container spacing={2}>
            <Grid item xs={6} md={3}>
              <div className='dt_Modal-FilterList'>
                {generateFilterAccordions(filterArr, checkedItems, handleCheckboxChange)}
              </div>
            </Grid>
            <Grid item xs={6} md={9}>
              <div className='dt_Modal-cardList'>
                <Grid container spacing={2}>
                  {filteredItems?.map(product => (
                    <Grid item key={product.id} xs={12} sm={4} md={4}>
                      <Card className='dt_cartListCard' sx={{ maxWidth: 250, position: 'relative' }}>
                        <CardActionArea>
                          <CardMedia
                            component="img"
                          ...
                          <CardContent>
                            ...
                            <div className='designNocartList'>
                              <p className='dt_DesignNoTExt'>{product?.designno}</p>
                            </div>
          ...
          <CardActions className='dt_projectUpdateCartBtn-group'>
            <div className="dt_projectUpdateCartBtn-group">
              <button className="dt_cartUpdate-btn" onClick={() => onUpdateCart(filteredItems)}>
                Update
              </button>
              <button className="dt_cartCancel-btn" onClick={onCancelCart}>
                Cancel
              </button>
            </div>
          </CardActions>
